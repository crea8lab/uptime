...
const helpers = {
  ...
  // Create an alphanumeric string of a given length
  createRandomString: (strLength) => {
    strLength = typeof(strLength) === 'number' && strLength > 0 ? strLength : false

    if (strLength) {
      // Define all possible characters
      const stringSet = 'abcdefghijklmnopqrstuvwxyz0123456789'

      let str = ''

      for (let i = 0; i <= strLength; i++) {
        const randomStr = stringSet.charAt(Math.floor(Math.random() * stringSet.length))
        str += randomStr
      }
